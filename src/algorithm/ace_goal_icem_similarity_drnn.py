...
class DRNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self._Q1, self._Q2 = q_goal(cfg), q_goal(cfg)
    ...
    def pred_z(self, z):
        """Predict next latent state representation."""
...
class AceDRNN:
    ...
    def plan(self, obs, hidden, eval_mode=False, step=None, t0=True):
        ...
        std = 0.5 * torch.ones(self.plan_horizon, self.cfg.action_dim, device=self.device)  # dummy init
        ...
        state, _ = torch.split(obs, [self.cfg.obs_shape[0], self.cfg.goal_dim], dim=-1)
        z = self.model.h(state)
        ...
        for i in range(self.cfg.iterations):
            ...
            if i == 0 and self.cfg.shift_elites_over_time and hasattr(self, '_elite_actions') and not t0:
                ...
                if extend_horizon:
                    last_actions = reused_actions[-2:]
                else:
                    last_actions = reused_actions[-1:]
            ...
            _std = torch.sqrt(torch.sum(score.unsqueeze(0) * (elite_actions - _mean.unsqueeze(1)) ** 2, dim=1) / (
                    score.sum(0) + 1e-9))
            _std = _std.clamp_(self.std, 2)  # if i > 0 else _std.clamp_(0.5, 2)
    ...
    def plan_cem(self, obs, hidden, eval_mode=False, step=None, t0=True):
        ...
        plan_metrics = {'external_reward_mean': 0.0, 'current_std': 0.0}
        if step < self.cfg.seed_steps and not eval_mode:
            return torch.empty(self.cfg.action_dim, dtype=torch.float32, device=self.device).uniform_(-1,
                                                                                                      1), None, plan_metrics
    ...
    def update_pi_bc(self, zs, input_zs, actions):
        ...
        self.pi_optim.step()
        self.model.track_q_grad(True)
        return pi_loss.item(), Q.abs().mean().item()
    ...
    def update(self, replay_buffer, step):
        ...
        state, _ = torch.split(obs, [self.cfg.obs_shape[0], self.cfg.goal_dim], dim=-1)
        next_state, _ = torch.split(next_obses, [self.cfg.obs_shape[0], self.cfg.goal_dim], dim=-1)
        ...
        # Representation
        z = self.model.h(state)
        next_zs = self.model_target.h(next_state)
        online_next_zs = self.model.h(next_state)
        ...
        similarity_loss, reward_loss, value_loss, priority_loss, q_value = 0, 0, 0, 0, 0
        ...
        if self.cfg.alpha_bc > 0:
            pi_loss, q_value = self.update_pi_bc(zs, z_traj.detach(), action)
        else:
