...
from envs.quad_envs import make_quadrotor_env_multi
from src.algorithm.ace_icem_similarity_drnn import AceDRNN
from src.algorithm.helper import Episode, RolloutBuffer
import logger
...
def evaluate_quad(env, agent, num_episodes, step, env_step, video):
...
def train(cfg):
    ...
    while iters < cfg.train_steps:
        ...
        if (episode_idx - 1) % cfg.eval_freq_episodes == 0:
            ...
            # common_metrics['episode_reward'] = eval_quad(env, agent, cfg.eval_episodes, iters, env_step, L.video)
